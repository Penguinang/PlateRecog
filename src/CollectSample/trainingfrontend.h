#ifndef TRAININGFRONTEND_H
#define TRAININGFRONTEND_H
...
namespace  Ui{
    class trainingFrontEnd;
}

class TrainWorker;

class trainingFrontEnd: public QMainWindow
{
    Q_OBJECT

public:
  explicit trainingFrontEnd(QWidget *parent = nullptr);
  ~trainingFrontEnd();
...
class TrainWorker : public QObject {
  Q_OBJECT
public:
  TrainWorker(trainingFrontEnd *trainingfrontend) : trainingfrontend(trainingfrontend){};

private:
  trainingFrontEnd *trainingfrontend = nullptr;
...
private:
  void train(trainingFrontEnd *mainWindow);
};

#endif // TRAININGFRONTEND_H
