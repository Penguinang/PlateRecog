  ...
  public:
    static PlateInfo GetPlateInfo(PlateInfo &plateInfo, PlateColor_t plateColor,
        ...
        auto combineVerticalOrigin = [](vector<CharInfo> &charInfos,
                                        PlateInfo &plateInfo) {
            for (size_t index = 1; index < min(charInfos.size(), 3ul);
                 ++index) {
                ...
                if ((intersectionLength = min(r1, r2) - max(l1, l2)) > 0) {
                    ...
                    if (p1 > 0.5 || p2 > 0.5) {
                        int t1 = rect.y, b1 = rect.y + rect.height,
                            t2 = lastRect.y, b2 = lastRect.y + lastRect.height;
                        lastRect = {min(l1, l2), min(t1, t2),
                                    max(r1, r2) - min(l1, l2),
                                    max(b1, b2) - min(t1, t2)};
                        lastCharInfo.OriginalMat =
                            plateInfo.OriginalMat(lastRect);
        ...
        auto rectCenter = [](const Rect &rect) {
            return cv::Point2i{rect.x + rect.width / 2,
        ...
        auto checkStartChineseCharacter = [&](PlateInfo &result) {
            ...
            if (charInfos.size() == 6) {
                ...
                float meanWidth = std::accumulate(
                    charInfos.begin(), charInfos.end(), 0.f,
                    [](float init, const CharInfo &next) -> float {
                ...
                if (firstRecoginzedChar >= PlateChar_t::BeiJing &&
                    ...
                    result.CharInfos.insert(
                        ...
                        {firstRecoginzedChar,
                         plateInfo.OriginalMat(firstCharRect), firstCharRect,
                         PlateLocateMethod_t::Unknown, splitMethod});
                }
            }
        };
