...
namespace PlateRecogn {
    class PlateRecognition_V3{
        ...
        // 返回值可能是null，改成指针
        public:
        static shared_ptr<PlateInfo> GetPlateInfoByMutilMethodAndMutilColor (const PlateInfo &plateInfo) {
        ...
        public:
        static PlateInfo GetPlateInfoByMutilMethod (const PlateInfo &plateInfo, PlateColor_t plateColor) {
            ...
            if (plateInfoByOriginal.CharInfos.empty() && plateInfoByOriginal.CharInfos.size() != 0) {
                plateInfos.push_back (plateInfoByOriginal);
            }
            if (plateInfoByGamma.CharInfos.empty() && plateInfoByGamma.CharInfos.size() != 0) {
                plateInfos.push_back (plateInfoByGamma);
            }
            if (plateInfoByIndex.CharInfos.empty() && plateInfoByIndex.CharInfos.size() != 0) {
                plateInfos.push_back (plateInfoByIndex);
            }
            if (plateInfoByLog.CharInfos.empty() && plateInfoByLog.CharInfos.size() != 0) {
        ...
        public:
        static PlateInfo GetPlateInfo (const PlateInfo &plateInfo, PlateColor_t plateColor, CharSplitMethod_t splitMethod) {
